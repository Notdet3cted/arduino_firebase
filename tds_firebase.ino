...
void setup(){
 ...
 res = wm.autoConnect("AutoConnectAP","password"); // password protected ap
   ...
   if(!res) {
       ...
       lcd.print("Connected...yeey :)");delay(1000);
   }
  ...
  delay(3000);
  lcd.clear();
  lcd.print("INISIALISASI");delay(500);
  lcd.print(".");delay(500);
  lcd.print(".");delay(500);
  lcd.print(".");delay(500);
  lcd.print(".");delay(500);
...
void loop(){
  ...
  if(millis()-printTimepoint > 800U){
    ...
    for(copyIndex=0; copyIndex<SCOUNT; copyIndex++){
      ...
      Firebase.RTDB.setInt(&fbdo, F("/data/PPM"), tdsValue) ? "ok" : fbdo.errorReason().c_str(); // Add ppm values
...
//   }
  ...
  if(tdsValue <= 500)
  {
    ...
    Firebase.RTDB.setInt(&fbdo, F("/data/kondisi"), "kurang") ? "ok" : fbdo.errorReason().c_str(); // Add kondisi Kurang
  ...
  {
    ket = "CUKUP NUTRISI";
    Firebase.RTDB.setInt(&fbdo, F("/data/status"), false) ? "ok" : fbdo.errorReason().c_str(); // Add status value OFF
